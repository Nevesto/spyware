...
func FindCookieFile() (string, error) {
	...
	err = filepath.Walk(braveUserDataPath, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			return nil
		}
	...
	if err != nil {
		return "", fmt.Errorf("error walking Brave user data path: %w", err)
	}

	if cookieFile == "" {
		return "", fmt.Errorf("Brave Cookies file not found")
	}
