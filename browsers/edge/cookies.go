...
func FindCookieFile() (string, error) {
	...
	err = filepath.Walk(edgeUserDataPath, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			return nil
		}
	...
	if err != nil {
		return "", fmt.Errorf("error walking Edge user data path: %w", err)
	}

	if cookieFile == "" {
		return "", fmt.Errorf("Edge Cookies file not found")
	}
